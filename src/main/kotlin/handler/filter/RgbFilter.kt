...
import top.e404.skiko.handler.ahsb
import top.e404.skiko.handler.handlePixel

object RgbFilter : ImageHandler {
    override suspend fun handle(bytes: ByteArray, data: ExtraData?): HandlerResult {
            ...
            frames.apply {
                ...
                runBlocking(IO) {
                    forEachIndexed { index, frame ->
                        launch {
                            frame.handleAsImage(index, size, FloatData(v * index), ::handleFrame)
                        }
                    }
                }
            }.encodeToBytes()
